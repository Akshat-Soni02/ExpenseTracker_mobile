...
const EmailInputScreen: React.FC = () => {
  ...
  const { contacts } = useLocalSearchParams<{ contacts?: string }>();
  const [addFriends, { isLoading }] = useAddUserFriendsMutation();

  const parsedContacts: ParsedContact[] = contacts ? JSON.parse(contacts) : [];

  const [emailData, setEmailData] = useState<Record<string, string>>(
    parsedContacts.reduce((acc: Record<string, string>, contact) => {
      acc[contact.recordID] = "";
      return acc;
    }, {})
  );

  const handleEmailChange = (recordID: string, email: string) => {
    setEmailData((prev) => ({
      ...prev,
      [recordID]: email,
    }));
    ...
    <View style={{ flex: 1, backgroundColor: "#fff", padding: 12 }}>
      ...
      <TouchableOpacity
        onPress={() =>
          Alert.alert(
            "Add Friends",
            "This will send an email to all your friends who are not currently on ExpenseEase!",
            [
